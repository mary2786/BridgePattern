using BridgePattern.MetodosEnvio;

namespace BridgePattern.Empresas
{
    public class Dhl : IEmpresa
    {
        private readonly IEnvio _envio;
        public Dhl(IEnvio envio)
        {
            _envio = envio;
        }

        public string Entregar()
        {
            string mensaje = _envio.Descargar();
            return string.Format("Paquete entregado por Dhl, {0}", mensaje);
        }

        public string Enviar()
        {
            string mensaje = _envio.Enviar();
            return string.Format("Paquete enviado por Dhl, {0}", mensaje);
        }

        public string Recoger()
        {
            string mensaje = _envio.Cargar();
            return string.Format("Paquete recogido por Dhl, {0}", mensaje);
        }
    }
}
